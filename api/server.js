...
var removeMedicine = async ({ medicineId, sellerId }) => {
  const medicine = await prisma.medicine.findUnique({
    where: { id: medicineId },
  });
  if (!medicine) throw new Error("Medicine not found");
...
var getMedicineDetails2 = async (req, res) => {
  try {
    ...
    const reviewPage = req.query.reviewPage
      ? parseInt(req.query.reviewPage)
      : 1;
    const reviewLimit = req.query.reviewLimit
      ? parseInt(req.query.reviewLimit)
      : 5;
...
var createOrderFromCart = async ({ userId, addressId, customerNote }) => {
...
var updateOrderStatus = async ({ orderId, status, userRole, userId }) => {
  return await prisma.$transaction(async (tx) => {
...
var getMyAddressById2 = async (req, res) => {
  try {
    ...
    const address = await addressService.getMyAddressById(user.id, addressId);
    return res.status(200).json({
      success: true,
...
var createCartItem = async ({ userId, medicineId, quantity = 1 }) => {
  return await prisma.$transaction(async (tx) => {
